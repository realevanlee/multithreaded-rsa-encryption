...
struct ThreadControl {
    ...
    CircularBuffer<pair<int, vector<string>>, 100> workQ;
    ...
    //deleted constructors because I don't want the controller to be copyable or removable
};
...
int main() {
    ...
    while(!lines.empty()) {
        ...
        if(!tmp.empty()) {
            {
                unique_lock lock(tc.m);
                tc.workQ.push_back({offset, tmp});
                cout << "pushing text chunk #" << offset++ << endl;
            }
            tc.ready.notify_all(); //notify that i have work to be done
        } else {
            break;
        }
